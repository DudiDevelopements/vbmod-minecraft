package com.dudi.vbmod.blocks.tileentities;

import com.dudi.vbmod.blocks.VibraniumFurnace;
import com.dudi.vbmod.recipes.vbfurnace.VibraniumFurnaceRecipes;
...
public class TileEntityVibraniumFurnace extends TileEntity implements ITickable {
    ...
    public static boolean isBurning(TileEntityVibraniumFurnace te) {
    ...
    public void update() {
        if (this.isBurning()) {
            --this.burnTime;
            VibraniumFurnace.setState(true, world, pos);
        }
        ...
        if (this.isBurning() && this.canSmelt() && cookTime > 0) {
            ...
            if (this.canSmelt() && this.isBurning()) {
                ItemStack output = VibraniumFurnaceRecipes.getInstance().getSinteringResult(inputs[0], inputs[1]);
                if (!output.isEmpty()) {
                    smelting = output;
                    cookTime++;
                    inputs[0].shrink(1);
    ...
    private boolean canSmelt() {
        if (((ItemStack) this.handler.getStackInSlot(0)).isEmpty() || ((ItemStack) this.handler.getStackInSlot(1)).isEmpty())
            ...
            ItemStack result = VibraniumFurnaceRecipes.getInstance().getSinteringResult((ItemStack) this.handler.getStackInSlot(0), (ItemStack) this.handler.getStackInSlot(1));
